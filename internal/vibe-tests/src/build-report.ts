 ...
 *
 * These are generated by `yarn build` but may not exist if the repo
 * was freshly cloned or only `yarn install` was run.
 */
function ensureCssDist(): void {
  ...
  if (missing.length > 0) {
    ...
    if (!fs.existsSync(xdsCss)) {
      execSync('yarn workspace @xds/core build', {
        cwd: REPO_ROOT,
        stdio: 'inherit',
      });
    }
    ...
    if (!fs.existsSync(themeCss)) {
      execSync('yarn workspace @xds/theme-default build', {
        cwd: REPO_ROOT,
        stdio: 'inherit',
      });
    }
