...
export const docs = {
  ...
  components: [
    ...
    },
    {
      ...
      description: 'Composable metadata row that sits below a chat bubble. Arranges timestamp, footer actions, and delivery status in a single line — direction auto-reverses for user messages. Pass it to the metadata prop on XDSChatMessageBubble. Renders nothing when all slots are empty.',
      props: [
        {name: 'timestamp', type: 'ReactNode', description: 'Timestamp content — typically an XDSTimestamp component with format="time" or format="relative".'},
        {name: 'footer', type: 'ReactNode', description: 'Footer actions or info — copy/retry/feedback buttons, model label, or any ReactNode. Use XDSHStack with ghost icon-only buttons for action rows.'},
        {name: 'status', type: "'sending' | 'sent' | 'delivered' | 'read' | 'error'", description: 'Delivery status indicator. Renders an icon and label — pulse animation for sending, red for error.'},
      ],
    },
    {
  ...
  usage: {
    ...
    bestPractices: [
      ...
      { guidance: true, description: 'Pass XDSChatMessageMetadata to the bubble\'s metadata prop. It auto-reverses direction for user vs. assistant messages so timestamps and status always read naturally.' },
      { guidance: true, description: 'Use XDSTimestamp with format="time" for recent messages and format="relative" for older ones. Avoid raw date strings — XDSTimestamp handles localization and live updates.' },
      { guidance: true, description: 'Keep footer actions in XDSChatMessageMetadata small and icon-only — ghost variant, size="sm" buttons grouped in an XDSHStack. Reserve footer actions for assistant messages.' },
      ...
      { guidance: false, description: 'Don\'t add footer actions to user messages — users don\'t need to copy, retry, or rate their own messages. Use status alone for delivery feedback.' },
      { guidance: false, description: 'Don\'t show delivery status on assistant messages — status (sending, delivered, read) only applies to outbound user messages.' },
    ],
    anatomy: [
...
export const docsZh = {
  ...
  components: [
    ...
    {
      name: 'XDSChatMessageMetadata',
      description: '可组合的消息元数据行。渲染时间戳、页脚操作和发送状态。用户消息方向自动反转。传递给气泡的 metadata 属性。',
      propDescriptions: {timestamp: '时间戳内容 — 通常是 XDSTimestamp 组件，使用 format="time" 或 format="relative"。', footer: '页脚操作或信息 — 复制/重试/反馈按钮、模型标签。使用 XDSHStack 组合 ghost 图标按钮。', status: '发送状态指示器。渲染图标和标签 — sending 有脉冲动画，error 为红色。'},
    },
    {
      name: 'XDSChatSystemMessage',
...
export const docsDense = {
  ...
  usage: {
    ...
    bestPractices: [
      ...
      { guidance: true, description: 'XDSChatMessageMetadata on bubble metadata prop — auto-reverses for user vs assistant.' },
      { guidance: true, description: 'XDSTimestamp format="time" for recent, format="relative" for older. No raw date strings.' },
      { guidance: true, description: 'Footer actions: ghost size="sm" icon-only buttons in XDSHStack. Assistant messages only.' },
      ...
      { guidance: false, description: 'Footer actions on user messages — use status alone for delivery feedback.' },
      { guidance: false, description: 'Delivery status on assistant messages — status is for outbound user messages only.' },
    ],
  },
  components: [
    {
    ...
    {
      name: 'XDSChatMessageMetadata',
      description: 'composable metadata row; timestamp · footer · status; auto-reverses for user sender; pass to bubble metadata prop',
      propDescriptions: {
        timestamp: 'XDSTimestamp w/ format="time" or "relative"',
        footer: 'actions/info: ghost sm icon-only buttons in XDSHStack, model label',
        status: 'delivery status; icon+label; pulse for sending, red for error',
      },
    },
    {
      name: 'XDSChatSystemMessage',
